...
/* Full-screen brand opener that plays on landing, then lifts to reveal the site.
   Monochrome (ink → beige). Skippable; instantly skipped under reduced-motion.
   To play it only once per browser session, gate on sessionStorage (see note). */

const EASE = [0.16, 1, 0.3, 1] as const;

// Ink palette for the opener overlay (its own dramatic dark tones).
const INK = "#14110b";
const CREAM = "#f4f0e6";
const HAIR = "#b3a789";
const META = "#8c8474";
...
export default function Intro() {
  ...
  useEffect(() => {
    // To show the opener only once per session, uncomment:
    ...
    <AnimatePresence>
      {!done && (
        ...
        >
          ...
          <motion.button
            onClick={() => setDone(true)}
            className="label absolute bottom-6 right-6 transition-colors"
            style={{ color: META }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
